...
class ImageMaskDatasetGenerator:
  ...
  def colorize_mask(self, colorized, mask, color):
  ...
  def generate(self, images_dir, masks_dir, output_images_dir, output_masks_dir):
    category_color_map = {"BE":(0,255,255), "cancer":(255,0,0), "HGD":(10,128,10), "polyp":(200,0,200), "suspicious":(40,40,255)}
    ...
    for image_file in image_files:
      ...
      for subdir in subdirs:
        ...
        mask_file  = os.path.join(subdir_fullath, nameonly + "_" + subdir + ".tif")
        if os.path.exists(mask_file):
          print("Found {}".format(mask_file))
          cat_mask =  cv2.imread(mask_file)
      
          color  = category_color_map[subdir]
          print("category {}  color {}".format(subdir, color))
          self.colorize_mask(colorized, cat_mask, color)
        else:
          pass
          
      out_filename = str(index) + ".png"
      ...
      if self.augmentation:
        self.augment(colorized, out_filename, output_masks_dir, border=(0, 0, 0), mask=True)
...
if __name__ == "__main__":
  try:
    ...
    masks_dir    = "./EDD2020/masksPerClass/"   

    output_dir   = "./Augmented-EDD2020-master/"
